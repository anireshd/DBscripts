...
WHERE   t.is_ms_shipped = 0 AND
		i.is_disabled = 0 AND
--		t.object_id = OBJECT_ID('Person.Person') AND	/* optionally filter by table */
        i.type_desc IN ( 'HEAP', 'CLUSTERED', 'NONCLUSTERED' );
GO

-- Grab the definition of each index (skip disabled indexes)
...
DECLARE @object_id INT, 
		...
		@included_cols VARCHAR(1000)
		
-- Open the cursor and retrieve first row's values
OPEN my_indexes;
...
BEGIN
    ...
    UPDATE  #indexes
    SET     included_cols = @included_cols
    WHERE   object_id = @object_id AND index_id = @index_id;

	SET @included_cols = NULL;

	-- Get the next row's values
    FETCH my_indexes INTO @object_id, @index_id;
END
...
SELECT  SCHEMA_NAME(i.schema_id) + '.' + OBJECT_NAME(i.object_id) AS 'schema.table', i.name, i.index_id, i.type_desc, 
		CASE i.is_primary_key WHEN 1 THEN 'PK' ELSE '' END AS 'is_primary_key', 
		CASE i.is_unique WHEN 1 THEN 'UNIQUE' ELSE '' END AS 'is_unique',
        i.key_cols, i.included_cols, i.fill_factor, i.filter_definition,
		STR(s.avg_fragmentation_in_percent, 5,2) AS 'avg_frag', s.page_count
FROM    #indexes i
		INNER JOIN sys.dm_db_index_physical_stats(DB_ID(), NULL, NULL, NULL, NULL) s 
